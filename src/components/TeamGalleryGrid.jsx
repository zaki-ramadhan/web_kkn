...
export default function TeamGalleryGrid({
  ...
  return (
    <div
      className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-5 ${className}`}
    >
      {members.map((member) => (
        <div
          key={member.id}
          className="group relative bg-white border border-slate-200/90 hover:border-emerald-400/60 rounded-2xl p-3.5 sm:p-4 transition-all duration-300 ease-out shadow-card-depth hover:shadow-card-hover hover:-translate-y-1 flex flex-col justify-between overflow-hidden"
        >
          ...
          <div className="relative aspect-square w-full rounded-xl overflow-hidden mb-3 bg-slate-100 border border-slate-100">
            ...
            />
            {member.isContact && (
              <span className="absolute bottom-2 left-2 px-2 py-0.5 rounded-md bg-brand-950/80 backdrop-blur-xs text-[10px] font-bold text-lime-400 border border-lime-400/30">
                Narahubung
              </span>
            )}
          </div>
